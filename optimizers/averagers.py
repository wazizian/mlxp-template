...
def get_pda_multi_avg_fn(gamma=8.0):
...
def build_pda(gamma: float = 8.0):
    return lambda model: optim.swa_utils.AveragedModel(
        model, multi_avg_fn=get_pda_multi_avg_fn(gamma=gamma), use_buffers=True
    )


def build_ema(decay: float = 0.999):
    return lambda model: optim.swa_utils.AveragedModel(
        model, avg_fn=optim.swa_utils.get_ema_multi_avg_fn(decay=decay), use_buffers=True
    )
